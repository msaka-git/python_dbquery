...
class server():
    def __init__(self, hostname, os, scope, type, ip, nat):
    ...
    def __str__(self):
        return "Hostname: {}\nOS: {}\nScope: {}\nType: {}\nIP: {}\nNat: {}".format(self.hostname, self.os, self.scope,
                                                                                   self.type, self.ip, self.nat)
...
class base():
    ...
    def connection_set(self):
        ...
        query = "create table if not exists servers (hostname TEXT,os TEXT,scope TEXT, type TEXT,ip INT, nat INT)"
    ...
    def show_servers(self):
        ...
        if (len(servers) == 0):
            ...
            for i in servers:
                Servers = server(i[0], i[1], i[2], i[3], i[4], i[5])
                print(Servers)
    ...
    def server_query(self, hostname):
        ...
        if (len(servers) == 0):
            ...
            Servers = server(servers[0][0], servers[0][1], servers[0][2], servers[0][3], servers[0][4], servers[0][5])
            print(Servers)

    def server_add(self, server):
        query = "insert into servers values(?,?,?,?,?,?)"
        self.cursor.execute(query, (server.hostname, server.os, server.scope, server.type, server.ip, server.nat))
        self.connection.commit()
